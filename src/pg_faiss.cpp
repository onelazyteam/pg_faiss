...
ensure_registry(void)
{
    if (pg_faiss_registry == NULL)
    {
        ...
        pg_faiss_registry = hash_create("pg_faiss index registry",
                                        128,
                                        &ctl,
        ...
        if (pg_faiss_registry == NULL)
            ereport(ERROR,
                    (errcode(ERRCODE_OUT_OF_MEMORY),
                     errmsg("failed to create pg_faiss registry")));
    }
}
...
static inline const char *
index_type_name(int index_type)
{
    switch (index_type)
    {
...
static inline faiss::MetricType
to_faiss_metric(int metric)
{
    if (metric == PG_FAISS_METRIC_L2)
        return faiss::METRIC_L2;

    return faiss::METRIC_INNER_PRODUCT;
}

static inline PgFaissIndexEntry *
lookup_entry(const char *name)
{
    if (pg_faiss_registry == NULL)
        return NULL;

    return (PgFaissIndexEntry *) hash_search(pg_faiss_registry,
                                             name,
                                             HASH_FIND,
                                             NULL);
}

static inline faiss::Index *
unwrap_idmap(faiss::Index *index)
{
...
static inline void
normalize_one(float *vec, int dim)
{
    float norm = 0.0f;
...
static inline void
normalize_many(float *vecs, int64 n, int dim)
{
    for (int64 i = 0; i < n; i++)
        normalize_one(&vecs[i * dim], dim);
}

static inline faiss::Index *
active_index(PgFaissIndexEntry *entry)
{
#ifdef USE_FAISS_GPU
...
#ifdef USE_FAISS_GPU
static void
rebuild_gpu_index(PgFaissIndexEntry *entry)
{
    if (entry->device != PG_FAISS_DEVICE_GPU)
        return;

    if (entry->gpu_resources == NULL)
        entry->gpu_resources = new faiss::gpu::StandardGpuResources();
    ...
    entry->gpu_index = faiss::gpu::index_cpu_to_gpu(entry->gpu_resources,
                                                     entry->gpu_device,
                                                     entry->cpu_index);
}

static void
sync_gpu_to_cpu(PgFaissIndexEntry *entry)
{
    if (entry->device != PG_FAISS_DEVICE_GPU || entry->gpu_index == NULL)
        return;
...
static inline void
free_entry_resources(PgFaissIndexEntry *entry)
{
#ifdef USE_FAISS_GPU
    if (entry->gpu_index != NULL)
    {
...
static JsonbValue *
jsonb_find_key(Jsonb *json, const char *key)
{
    ...
    return findJsonbValueFromContainer(&json->root,
                                       JB_FOBJECT,
                                       &key_value);
}

static bool
jsonb_get_int32(Jsonb *json, const char *key, int32 *out)
{
    ...
    {
        *out = DatumGetInt32(DirectFunctionCall1(numeric_int4,
                   ...
                   int32 max_value)
{
    ...
    if (!jsonb_get_int32(json, key, &value))
        return default_value;

    if (value < min_value || value > max_value)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("option \"%s\" out of range (%d..%d): %d",
                        key, min_value, max_value, value)));

    return value;
}

static inline PgVector *
datum_to_pgvector(Datum datum)
{
    return (PgVector *) PG_DETOAST_DATUM(datum);
}

static void
read_vector_array(ArrayType *arr,
                  int expected_dim,
            ...
            ereport(ERROR,
                    (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                     errmsg("vector dimension mismatch: expected %d, got %d",
                            expected_dim, vec->dim)));

        memcpy(&out[(size_t) i * (size_t) expected_dim],
               vec->x,
               sizeof(float) * expected_dim);
    }
...
static void
read_ids_array(ArrayType *arr, std::vector<faiss::idx_t> &ids)
{
    Datum  *elements = NULL;
    bool   *nulls = NULL;
    int     nelems = 0;
    ...
    if (ARR_ELEMTYPE(arr) != INT8OID)
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 errmsg("ids argument must be bigint[]")));
    ...
    if (nelems <= 0)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("ids argument must not be empty")));
...
static void
apply_search_params(PgFaissIndexEntry *entry,
                    faiss::Index *index,
                    Jsonb *search_params,
                    int *old_ef_search,
                    int *old_nprobe,
                    ...
                    bool *changed_nprobe)
{
    ...
    *changed_nprobe = false;
    ...
    if (entry->index_type == PG_FAISS_INDEX_IVF_FLAT ||
...
static void
restore_search_params(faiss::Index *index,
                      int old_ef_search,
                      int old_nprobe,
                      bool changed_ef_search,
                      bool changed_nprobe)
{
    ...
    if (changed_ef_search)
    {
        ...
        if (hnsw != NULL)
            hnsw->hnsw.efSearch = old_ef_search;
    }
...
static void
materialize_result_begin(FunctionCallInfo fcinfo,
...
static void
materialize_result_end(ReturnSetInfo *rsinfo,
                       Tuplestorestate *tupstore,
                       TupleDesc tupdesc)
{
...
static void
write_metadata_file(const PgFaissIndexEntry *entry, const char *path)
{
...
read_metadata_file(const char *path)
{
...
{
    if (pg_strcasecmp(metric, "l2") == 0)
        return PG_FAISS_METRIC_L2;
    if (pg_strcasecmp(metric, "ip") == 0 || pg_strcasecmp(metric, "inner_product") == 0)
        return PG_FAISS_METRIC_IP;
    if (pg_strcasecmp(metric, "cosine") == 0)
        return PG_FAISS_METRIC_COSINE;

    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             errmsg("unknown metric: %s", metric)));
...
{
    if (pg_strcasecmp(index_type, "hnsw") == 0)
    ...
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             errmsg("unknown index_type: %s", index_type)));

    return PG_FAISS_INDEX_HNSW;
}

static inline int
parse_device(const char *device)
{
    if (pg_strcasecmp(device, "cpu") == 0)
        return PG_FAISS_DEVICE_CPU;
    if (pg_strcasecmp(device, "gpu") == 0)
        return PG_FAISS_DEVICE_GPU;

    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             errmsg("unknown device: %s", device)));

    return PG_FAISS_DEVICE_CPU;
}

static faiss::Index *
build_index(const PgFaissIndexEntry *entry)
{
    ...
    if (entry->index_type == PG_FAISS_INDEX_HNSW)
    {
        faiss::IndexHNSWFlat *index = new faiss::IndexHNSWFlat(entry->dim,
                                                               entry->hnsw_m,
                                                               metric);
    ...
    else if (entry->index_type == PG_FAISS_INDEX_IVF_FLAT)
    {
        ...
        faiss::IndexIVFFlat *index = new faiss::IndexIVFFlat(quantizer,
                                                             entry->dim,
                                                             entry->ivf_nlist,
                                                             metric);
    ...
    {
        faiss::IndexFlat *quantizer = new faiss::IndexFlat(entry->dim, metric);
        faiss::IndexIVFPQ *index = new faiss::IndexIVFPQ(quantizer,
                                                         entry->dim,
                                                         entry->ivf_nlist,
                                                         entry->ivfpq_m,
                                                         entry->ivfpq_bits,
...
extern "C" Datum
pg_faiss_index_create(PG_FUNCTION_ARGS)
{
    ...
    entry = (PgFaissIndexEntry *) hash_search(pg_faiss_registry,
                                              name,
                                              HASH_ENTER,
                                              &found);
    ...
    if (found)
        ereport(ERROR,
                (errcode(ERRCODE_DUPLICATE_OBJECT),
                 errmsg("index \"%s\" already exists", name)));
    ...
    entry->hnsw_m = jsonb_option_int32(options,
                                       "m",
                                       PG_FAISS_DEFAULT_HNSW_M,
                                       2,
                                       256);
    entry->hnsw_ef_construction = jsonb_option_int32(options,
                                                     "ef_construction",
                                                     PG_FAISS_DEFAULT_HNSW_EF_CONSTRUCTION,
                                                     4,
                                                     1000000);
    entry->hnsw_ef_search = jsonb_option_int32(options,
                                               "ef_search",
                                               PG_FAISS_DEFAULT_HNSW_EF_SEARCH,
                                               1,
                                               1000000);
    entry->ivf_nlist = jsonb_option_int32(options,
                                          "nlist",
                                          PG_FAISS_DEFAULT_IVF_NLIST,
                                          1,
                                          1000000);
    entry->ivf_nprobe = jsonb_option_int32(options,
                                           "nprobe",
                                           PG_FAISS_DEFAULT_IVF_NPROBE,
                                           1,
                                           1000000);
    entry->ivfpq_m = jsonb_option_int32(options,
                                        "pq_m",
                                        PG_FAISS_DEFAULT_IVFPQ_M,
                                        1,
                                        4096);
    entry->ivfpq_bits = jsonb_option_int32(options,
                                           "pq_bits",
                                           PG_FAISS_DEFAULT_IVFPQ_BITS,
                                           1,
                                           16);
    entry->gpu_device = jsonb_option_int32(options,
                                           "gpu_device",
                                           0,
                                           0,
                                           128);
...
extern "C" Datum
pg_faiss_index_train(PG_FUNCTION_ARGS)
{
...
extern "C" Datum
pg_faiss_index_add(PG_FUNCTION_ARGS)
{
    char *name = text_to_cstring(PG_GETARG_TEXT_PP(0));
    ...
    catch (const std::exception &e)
    {
        ereport(ERROR,
                (errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
                 errmsg("FAISS add error: %s", e.what())));
    }
...
extern "C" Datum
pg_faiss_index_search(PG_FUNCTION_ARGS)
{
    char *name = text_to_cstring(PG_GETARG_TEXT_PP(0));
    ...
    try
    {
        ...
        if (effective_k > 0)
        {
            ...
            if (entry->metric == PG_FAISS_METRIC_COSINE)
                normalize_one(query_buf.data(), entry->dim);

            apply_search_params(entry,
                                index,
                                search_params,
                                &old_ef_search,
                                &old_nprobe,
...
extern "C" Datum
pg_faiss_index_search_batch(PG_FUNCTION_ARGS)
{
    char *name = text_to_cstring(PG_GETARG_TEXT_PP(0));
...
extern "C" Datum
pg_faiss_index_save(PG_FUNCTION_ARGS)
{
    char *name = text_to_cstring(PG_GETARG_TEXT_PP(0));
...
extern "C" Datum
pg_faiss_index_load(PG_FUNCTION_ARGS)
{
    char *name = text_to_cstring(PG_GETARG_TEXT_PP(0));
    char *path = text_to_cstring(PG_GETARG_TEXT_PP(1));
    ...
    try
    {
        ...
        entry->metric = loaded_index->metric_type == faiss::METRIC_L2 ?
                        PG_FAISS_METRIC_L2 : PG_FAISS_METRIC_IP;
        ...
        if (meta.find("metric") != meta.end())
            entry->metric = parse_metric(meta["metric"].c_str());
...
extern "C" Datum
pg_faiss_index_stats(PG_FUNCTION_ARGS)
{
    char *name = text_to_cstring(PG_GETARG_TEXT_PP(0));
    PgFaissIndexEntry *entry = lookup_entry(name);
    StringInfoData json;
    ...
    appendStringInfo(&json, "\"hnsw\":{\"m\":%d,\"ef_construction\":%d,\"ef_search\":%d},",
                     entry->hnsw_m,
                     entry->hnsw_ef_construction,
                     entry->hnsw_ef_search);
    appendStringInfo(&json, "\"ivf\":{\"nlist\":%d,\"nprobe\":%d},",
                     entry->ivf_nlist,
                     entry->ivf_nprobe);
    appendStringInfo(&json, "\"ivfpq\":{\"m\":%d,\"bits\":%d},",
                     entry->ivfpq_m,
                     entry->ivfpq_bits);
...
extern "C" Datum
pg_faiss_index_drop(PG_FUNCTION_ARGS)
{
    char *name = text_to_cstring(PG_GETARG_TEXT_PP(0));
    PgFaissIndexEntry *entry;
...
extern "C" Datum
pg_faiss_reset(PG_FUNCTION_ARGS)
{
