CREATE EXTENSION vector;
CREATE EXTENSION pg_faiss;

SELECT pg_faiss_reset() IS NOT NULL AS reset_ok;

SELECT pg_faiss_index_create(
    'idx_h',
    4,
    'l2',
    'hnsw',
    '{"m":16,"ef_construction":128,"ef_search":128}'::jsonb,
    'cpu'
) IS NOT NULL AS create_hnsw_ok;

SELECT pg_faiss_index_add(
    'idx_h',
    ARRAY[1,2,3,4]::bigint[],
    ARRAY[
        '[1,0,0,0]'::vector,
        '[2,0,0,0]'::vector,
        '[3,0,0,0]'::vector,
        '[4,0,0,0]'::vector
    ]::vector[]
) AS added_hnsw;

SELECT (pg_faiss_index_stats('idx_h')->>'num_vectors')::int AS hnsw_num_vectors;

SELECT id
FROM pg_faiss_index_search('idx_h', '[1,0,0,0]'::vector, 2, '{}'::jsonb)
ORDER BY distance, id;

SELECT count(*) AS batch_rows
FROM pg_faiss_index_search_batch(
    'idx_h',
    ARRAY['[1,0,0,0]'::vector, '[4,0,0,0]'::vector]::vector[],
    2,
    '{}'::jsonb
);

SELECT pg_faiss_index_save('idx_h', '/tmp/pg_faiss_regress.idx') IS NOT NULL AS save_ok;
SELECT pg_faiss_index_drop('idx_h') IS NOT NULL AS drop_hnsw_ok;
SELECT pg_faiss_index_load('idx_h', '/tmp/pg_faiss_regress.idx', 'cpu') IS NOT NULL AS load_ok;

SELECT id
FROM pg_faiss_index_search('idx_h', '[1,0,0,0]'::vector, 1, '{}'::jsonb);

SELECT pg_faiss_index_create(
    'idx_ivf',
    4,
    'cosine',
    'ivfflat',
    '{"nlist":2,"nprobe":2}'::jsonb,
    'cpu'
) IS NOT NULL AS create_ivf_ok;

SELECT pg_faiss_index_train(
    'idx_ivf',
    ARRAY[
        '[1,0,0,0]'::vector,
        '[0,1,0,0]'::vector,
        '[0,0,1,0]'::vector,
        '[0,0,0,1]'::vector
    ]::vector[]
) IS NOT NULL AS train_ivf_ok;

SELECT pg_faiss_index_add(
    'idx_ivf',
    ARRAY[11,12,13,14]::bigint[],
    ARRAY[
        '[1,0,0,0]'::vector,
        '[0,1,0,0]'::vector,
        '[0,0,1,0]'::vector,
        '[0,0,0,1]'::vector
    ]::vector[]
) AS added_ivf;

SELECT id
FROM pg_faiss_index_search('idx_ivf', '[1,0,0,0]'::vector, 1, '{"nprobe":2}'::jsonb);

SELECT pg_faiss_index_drop('idx_h') IS NOT NULL AS drop_loaded_ok;
SELECT pg_faiss_index_drop('idx_ivf') IS NOT NULL AS drop_ivf_ok;
SELECT pg_faiss_reset() IS NOT NULL AS final_reset_ok;
